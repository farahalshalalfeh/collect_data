...
actions = np.arange(BASE_CLASS_ID, BASE_CLASS_ID + len(digits))  # 1..10

# =====================================
# Paths & Settings  (طلباتك الجديدة)
# =====================================
DATA_PATH = "mp_num"    # هنا ينحفظ keypoints
VIDEO_PATH = "num_ved"  # هنا تنحفظ الفيديوهات

TARGET_FPS = 30
SECONDS_PER_SEQUENCE = 3
sequence_length = TARGET_FPS * SECONDS_PER_SEQUENCE  # 90 فريم
videos_per_person = 25

SAVE_ANNOTATED_VIDEO = True  # True: يحفظ الفيديو مع landmarks
...
with mp_hands.Hands(
    ...
    for action in actions:
        ...
        print(f"\nClass {action} → Digit: {label} ({eng_name})")
